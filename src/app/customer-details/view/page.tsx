...
function ViewCustomerDetailsContent() {
  ...
  const fetchClientReceipts = useCallback(async () => {
    ...
    try {
      ...
      setReceipts(
        data.map((receipt: any) => ({
          ...
          issueDate: receipt.issueDate
            ? new Date(receipt.issueDate).toISOString()
            : null,
      ...
      console.error(
        `Error fetching receipts for client ID ${clientId}:`,
        error
      );
      toast({
        variant: "default",
  ...
  const fetchAdminReceipts = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        `Error fetching admin receipts for client ID ${clientId}:`,
        error
      );
      toast({
        variant: "default",
  ...
  const handleViewFullReceipt = (receipt: ClientReceipt) => {
    router.push(
      `/receipt/details?receiptId=${receipt.id}&clientId=${
        receipt.clientId
      }&clientName=${encodeURIComponent(receipt.clientName)}`
    );
  };

  const handleViewAdminReceipt = (receipt: AdminReceipt) => {
    router.push(
      `/admin-receipt/details?receiptId=${receipt.id}&clientId=${
        receipt.clientId
      }&clientName=${encodeURIComponent(receipt.clientName)}`
    );
  };
  ...
  const handleDeleteClient = async (clientToDelete: Client) => {
    try {
      ...
      toast({
        ...
        title: "Delete Failed",
        description:
          "There was a problem deleting the client. Please try again.",
      });
    }
  };
      ...
      <Layout>
        <div className="flex flex-col justify-center items-center min-h-screen p-4">
          <p className="text-destructive mb-4">
            Client details could not be loaded or client not found.
          </p>
      ...
      <div className="container mx-auto p-4 md:p-8">
        <Card className="mb-6">
          <CardHeader className="flex flex-row justify-between items-center">
            <div>
              ...
              <CardDescription>
                Shop: {client.shopName || "N/A"}. Client ID:{" "}
            ...
            <div className="flex gap-2">
              <Button
                onClick={() =>
                  router.push(`/customer-details/edit?clientId=${client.id}`)
                }
                variant="outline"
                size="sm"
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      ...
                      the client {client.clientName} and all associated data.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
          ...
          <CardContent>
            ...
            {client.createdAt && (
              <p className="text-sm text-muted-foreground">
                Client Since: {format(new Date(client.createdAt), "PPP")}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              View all receipts associated with this client
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loadingReceipts ? (
              ...
              <ScrollArea className="h-[30vh] w-full rounded-md border">
                <ul className="p-4 space-y-3">
                  {receipts.map((receipt) => {
                    ...
                    if (
                      receipt.issueDate &&
                      isValid(parseISO(receipt.issueDate))
                    ) {
                      formattedIssueDate = format(
                        parseISO(receipt.issueDate),
                        "PPP"
                      );
                    } else if (receipt.issueDate) {
                      // If it's a Date object but not string
                      try {
                        formattedIssueDate = format(
                    ...
                    return (
                      <li
                        key={receipt.id}
                        className="border rounded-md p-3 flex justify-between items-center bg-card hover:bg-muted/50"
                      >
                        <div>
                          <p className="font-medium">
                            Receipt ID: {receipt.id.substring(0, 10)}...
                          </p>
                          <p className="text-sm text-muted-foreground">
                          ...
                          <p className="text-sm">
                            Final Weight: {receipt.totals.finalWt.toFixed(3)}
                          </p>
                          <p className="text-sm">
                            Stone Amount: {receipt.totals.stoneAmt.toFixed(2)}
                          </p>
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleViewFullReceipt(receipt)}
                        >
                          <Eye className="mr-2 h-4 w-4" /> View Full Receipt
                        </Button>
                      </li>
                    );
                  })}
        ...
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>
              View all admin receipts associated with this client
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <ul className="p-4 space-y-3">
                  {adminReceipts.map((receipt) => {
                    const givenDate = receipt.given?.date
                      ? format(new Date(receipt.given.date), "PPP")
                      : "N/A";
                    const receivedDate = receipt.received?.date
                      ? format(new Date(receipt.received.date), "PPP")
                      : "N/A";
                    ...
                    return (
                      ...
                      >
                        <div>
                          <p className="font-medium">
                            Admin Receipt ID: {receipt.id.substring(0, 10)}...
                          </p>
                          ...
                          {receipt.given && (
                            <div className="text-sm">
                              ...
                              <p>
                                Given Total: {receipt.given.total.toFixed(3)}
                              </p>
                            </div>
                          )}
                          {receipt.received && (
                        ...
                        <div className="flex flex-col gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleViewAdminReceipt(receipt)}
                          >
                          ...
                          {receipt.status === "complete" && (
                            <Button
                              variant="default"
                              size="sm"
                              onClick={() => handleViewAdminBill(receipt)}
                            >
