...
function AdminDetailsListContent() {
  ...
  const fetchAdminReceipts = useCallback(async () => {
    ...
    console.warn(
      "Admin receipts fetching not implemented. Waiting for MongoDB setup."
    );
    toast({
      title: "Data Fetching Pending",
      description:
        "Admin receipt details will be loaded once MongoDB is configured.",
      variant: "default",
    });
    setReceipts([]);
    setLoading(false);
    ...
    <div className="container mx-auto p-4 md:p-8">
        ...
        <CardHeader className="flex flex-row justify-between items-center">
          <div>
            <CardTitle className="text-2xl">
              Admin Details - All Admin Receipts
            </CardTitle>
            <CardDescription className="mt-1">
              This page displays details of all admin receipts. Data will be
              loaded from MongoDB once configured.
            </CardDescription>
          </div>
      ...
      {loading ? (
          ...
          <div className="space-y-6">
            {receipts.map((receipt) => {
              const formattedDateGiven =
                receipt.given?.date && isValid(new Date(receipt.given.date))
                  ? format(new Date(receipt.given.date), "PPP")
                  : "N/A";
              const formattedDateReceived =
                receipt.received?.date &&
                isValid(new Date(receipt.received.date))
                          ...
                          <table className="min-w-full text-sm border-collapse border border-border">
                            ...
                            <tbody>
                              ...
                              <tr className="font-semibold bg-muted/50">
                                ...
                                <td className="p-2 border text-right">
                                  {getDisplayValue(
                                    receipt.given.totalPureWeight
                                  )}
                                </td>
                                <td className="p-2 border"></td>
                                <td className="p-2 border"></td>
                                <td className="p-2 border text-right">
                                  {getDisplayValue(receipt.given.total)}
                                </td>
                              </tr>
                            </tbody>
                          </table>
                          ...
                          <table className="min-w-full text-sm border-collapse border border-border">
                            ...
                            <tbody>
                              ...
                              <tr className="font-semibold bg-muted/50">
                                ...
                                <td className="p-2 border text-right">
                                  {getDisplayValue(
                                    receipt.received.totalOrnamentsWt
                                  )}
                                </td>
                                <td className="p-2 border text-right">
                                  {getDisplayValue(
                                    receipt.received.totalStoneWeight
                                  )}
                                </td>
                                <td className="p-2 border text-right">
                                  {getDisplayValue(
                                    receipt.received.totalSubTotal
                                  )}
                                </td>
                                <td className="p-2 border"></td>
                                <td className="p-2 border text-right">
                                  {getDisplayValue(receipt.received.total)}
                                </td>
                              </tr>
                            </tbody>
                          </table>
                    ...
                    {(!receipt.given || receipt.given.items.length === 0) &&
                      (!receipt.received ||
                        ...
                        <p className="p-4 text-muted-foreground">
                          This admin receipt is currently empty.
                        </p>
                      )}
                  </CardContent>
                </Card>
        ...
        <p className="text-muted-foreground text-center py-10">
          No admin receipts found. Waiting for MongoDB configuration.
        </p>
      )}
    </div>
  );
