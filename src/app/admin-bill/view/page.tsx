...
function AdminBillViewContent() {
    ...
    const fetchReceipt = async () => {
      if (!receiptId) {
        toast({
          variant: "destructive",
          title: "Error",
          description: "Admin Receipt ID is missing.",
        });
        setLoading(false);
      ...
      try {
        ...
        toast({
          ...
          description:
            "There was a problem loading receipt data. Please try again.",
        });
        router.push("/admin-bill");
      } finally {
        setLoading(false);
  ...
  const downloadReceipt = () => {
    if (!receiptData) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Cannot download receipt, data not loaded.",
      });
      return;
    }
    const hasGiven = receiptData.given && receiptData.given.items.length > 0;
    const hasReceived =
      receiptData.received && receiptData.received.items.length > 0;
    const givenDate = receiptData.given?.date
      ? new Date(receiptData.given.date)
      : null;
    const receivedDate = receiptData.received?.date
    ...
    if (!hasGiven && !hasReceived) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Cannot download an empty admin receipt.",
      });
      return;
    }
    ...
    startY += 6;
    const receiptCreationDate = receiptData.createdAt
      ? new Date(receiptData.createdAt)
      : null;
    if (receiptCreationDate && isValid(receiptCreationDate)) {
      doc.text(
        `Receipt Date: ${format(receiptCreationDate, "PPP p")}`,
        margin + 5,
        startY
      );
      startY += 6;
    ...
    if (hasGiven && receiptData.given) {
      const formattedGivenDate =
        givenDate && isValid(givenDate) ? format(givenDate, "PPP") : "N/A";
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text(
        `Given Details (Date: ${formattedGivenDate})`,
        margin + 5,
        startY
      );
      startY += 8;

      const givenTableColumns = [
        "S.No",
        "Product Name",
        "Pure Weight",
        "Pure %",
      ...
      const givenTotalRow = [
        ...
        {
          content: getDisplayValue(receiptData.given.totalPureWeight, 3),
          styles: { fontStyle: "bold" as const, halign: "right" as const },
        },
        "",
        "",
        {
          content: getDisplayValue(receiptData.given.total, 3),
          styles: { fontStyle: "bold" as const, halign: "right" as const },
        },
      ];
    ...
    if (hasReceived && receiptData.received) {
      ...
      const formattedReceivedDate =
        receivedDate && isValid(receivedDate)
          ? format(receivedDate, "PPP")
          : "N/A";
      doc.setFontSize(12);
      doc.setFont("helvetica", "bold");
      doc.text(
        `Received Details (Date: ${formattedReceivedDate})`,
        margin + 5,
        startY
      );
      startY += 8;
      ...
      const receivedTableRows = receiptData.received.items.map(
        (item, index) => [
          index + 1,
          item.productName || "",
      ...
      const receivedTotalRow = [
        ...
        {
          content: getDisplayValue(receiptData.received.totalOrnamentsWt, 3),
          styles: { fontStyle: "bold" as const, halign: "right" as const },
        },
        {
          content: getDisplayValue(receiptData.received.totalStoneWeight, 3),
          styles: { fontStyle: "bold" as const, halign: "right" as const },
        },
        {
          content: getDisplayValue(receiptData.received.totalSubTotal, 3),
          styles: { fontStyle: "bold" as const, halign: "right" as const },
        },
        "",
        {
          content: getDisplayValue(receiptData.received.total, 3),
          styles: { fontStyle: "bold" as const, halign: "right" as const },
        },
      ];
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        [
          `Received Total: (${operationText})`,
          getDisplayValue(finalReceivedTotal, 3),
        ],
    ...
    doc.save(
      `admin_receipt_${receiptData.clientName.replace(/\s+/g, "_")}_${format(
        new Date(),
        "yyyyMMdd"
      )}.pdf`
    );
    ...
    return (
      <Layout>
        <div className="flex justify-center items-center min-h-screen">
          <p>
            Loading admin receipt view... Waiting for MongoDB configuration.
          </p>
        </div>
      </Layout>
    );
      ...
      <Layout>
        <div className="flex flex-col justify-center items-center min-h-screen p-4">
          <p className="text-destructive mb-4">
            Admin receipt data could not be loaded or was not found.
          </p>
  ...
  const formattedDateGiven =
    receiptData.given?.date && isValid(new Date(receiptData.given.date))
      ? format(new Date(receiptData.given.date), "PPP")
      : "N/A";
  const formattedDateReceived =
    receiptData.received?.date && isValid(new Date(receiptData.received.date))
      ? format(new Date(receiptData.received.date), "PPP")
      : "N/A";
      ...
      <div className="container mx-auto p-4 md:p-8">
        <Card className="mb-6">
          <CardHeader className="flex flex-row justify-between items-center">
            <div>
              ...
              <CardDescription>
                Client: {receiptData.clientName} (ID:{" "}
                {receiptData.clientId.substring(0, 10)}...)
              </CardDescription>
            </div>
                  ...
                  <tbody>
                    {receiptData.given?.items?.map((item, index) => (
                      <tr key={`given-${index}`}>
                        ...
                        <td className="p-2 border text-right">
                          {getDisplayValue(item.pureWeight)}
                        </td>
                        <td className="p-2 border text-right">
                          {getDisplayValue(item.purePercent, 2)}
                        </td>
                    ...
                    {givenTotals && (
                      <tr className="bg-muted font-semibold">
                        ...
                        <td className="p-2 border text-right">
                          {getDisplayValue(givenTotals.totalPureWeight)}
                        </td>
                        <td className="p-2 border"></td>
                        <td className="p-2 border"></td>
                        <td className="p-2 border text-right">
                          {getDisplayValue(givenTotals.total)}
                        </td>
                      </tr>
                    )}
                  </tbody>
                  ...
                  <tbody>
                    {receiptData.received?.items?.map((item, index) => (
                      <tr key={`received-${index}`}>
                        ...
                        <td className="p-2 border">{item.productName}</td>
                        <td className="p-2 border text-right">
                          {getDisplayValue(item.finalOrnamentsWt)}
                        </td>
                        <td className="p-2 border text-right">
                          {getDisplayValue(item.stoneWeight)}
                        </td>
                        <td className="p-2 border text-right">
                          {getDisplayValue(item.subTotal)}
                        </td>
                        <td className="p-2 border text-right">
                          {getDisplayValue(item.makingChargePercent, 2)}
                        </td>
                    ...
                    {receivedTotals && (
                      <tr className="bg-muted font-semibold">
                        ...
                        <td className="p-2 border text-right">
                          {getDisplayValue(receivedTotals.totalOrnamentsWt)}
                        </td>
                        <td className="p-2 border text-right">
                          {getDisplayValue(receivedTotals.totalStoneWeight)}
                        </td>
                        <td className="p-2 border text-right">
                          {getDisplayValue(receivedTotals.totalSubTotal)}
                        </td>
                        <td className="p-2 border"></td>
                        <td className="p-2 border text-right">
                          {getDisplayValue(receivedTotals.total)}
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Final calculation based on Given and Received totals. This is for
              on-screen viewing and can be manually adjusted. This section is
              not saved and only for PDF output.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div>
              ...
              <Input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                  setGivenTotalInput(e.target.value)
                }
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e: ChangeEvent<HTMLInputElement>) =>
                  setReceivedTotalInput(e.target.value)
                }
